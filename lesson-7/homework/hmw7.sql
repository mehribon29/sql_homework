...
4.
select *from Products;
select count (distinct ProductName) as UniqueName
from Products

5.
select * from Products;
Select Sum (Stockquantity*Price) as TotalSales
from Products
Where ProductId=7
...
Select ProductID,
Sum (SaleAmount) as TOTALSales, 
avg(SaleAmount) as AvarageSales
from Sales
Group by ProductID;
...
select*from Sales;
select ProductID,
AVG(SaleAmount) as AveragePrice
from Sales
group by ProductID
